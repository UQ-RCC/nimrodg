...
public class ResourceClient {
	...
	public ResourceClient(RestOperations rest, URI api) {
		this.rest = rest;
		this.api = api;
	...
	public <T> ResponseEntity<T> executeJob(String task, String configuration, String host, int retries, Map<String, String> parameters, Class<T> clazz) {
		...
		UriComponentsBuilder urib = UriComponentsBuilder.fromUri(api)
				.pathSegment("api", "execute", task);

		if(configuration != null) {
			urib.pathSegment("in", configuration);
		}

		if(host != null) {
			urib.pathSegment("on", host);
		}
